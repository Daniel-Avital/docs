...
* **Active / Inactive**: Whether the guideline is currently enforced.
...
For setup details, see [Guidelines as Code](https://docs.pandorian.ai/essentials/pandorian-integrations#guidelines-as-code).
...
For setup details, see [Dynamic Context Providers](https://docs.pandorian.ai/essentials/pandorian-integrations#dynamic-context-providers).
...
In Pandorian, scoping is mainly managed through **Scan Policies**.

Navigate to **Scan Policies** from the left sidebar to configure where guidelines run.

Scan Policies help you:

* Ignore paths that should not be scanned.
* Scope guideline groups to specific files or directories.
* Apply organization-level defaults.
* Add repository-level overrides.
* Use guideline tags to decide which groups of guidelines apply where.

#### Organization-level policy

Organization-level policy applies across repositories by default.

Use organization-level policy for rules that should be consistent across the organization, such as ignoring generated files, vendor directories, or other paths that should not be scanned.

Common organization-level ignored paths include:
...
To add an ignored path, click **Ignore a path**.

#### Guideline scoping

Guideline scoping lets you run tagged groups of guidelines only where they are relevant.

Instead of running every guideline across every file, you can scope a guideline tag to a specific path.

Examples:

* Run security guidelines only against authentication code.
* Run data guidelines only against migration files.
* Run infrastructure guidelines only against deployment configuration.
* Run team-specific guidelines only against repositories owned by that team.

To add a scoping rule, click **Add scoping rule**.

#### Repository overrides

Repository overrides let you customize scan policy for a specific repository.

Use repository overrides when one repository has different conventions, contains legacy code, includes generated files in unusual paths, or requires different scoping than the organization default.

To add a repository override, click **Add repository override**.

Repository-level overrides take precedence over organization-level defaults.

#### Tags and scan policies

Guideline tags help organize guidelines and apply groups of guidelines through scan policies.

A tag can be created when you create or edit a guideline. Tags are free text, so each organization can define the taxonomy that makes sense for its teams and repositories.

For example, you might tag guidelines with:
...
Then, in Scan Policies, you can decide where guidelines with those tags should run.

Example:

* Guidelines tagged `auth` run only on `src/auth/**`.
* Guidelines tagged `data-storage` run only on `db/migrations/**`.
* Guidelines tagged `platform` run only on platform-owned repositories.

This reduces noise and helps enforce the right standards in the right parts of the codebase.

#### Common scoping examples

**Ignore generated files**

Generated files usually should not be reviewed against human-authored engineering standards. Add ignored path patterns for generated code so Pandorian skips them during scans.

**Ignore vendor directories**

Third-party or vendored code is usually not owned by your team. Add vendor paths to ignored paths so violations are not reported for code your team does not maintain.

**Apply security guidelines only to authentication services**

Tag relevant guidelines with `auth` or `security`, then scope that tag to authentication-related paths or repositories.

**Apply data guidelines only to migration files**

Tag data-related guidelines with `data-storage`, then scope them to migration folders.

**Override organization defaults for a legacy repository**

If a legacy repository needs different behavior, create a repository override instead of changing the organization-wide policy for every repository.
...
Keep Policy as Code in the Integrations guide as the technical source of truth, and use this Guides page as the conceptual overview.

For setup details, see [Policy as Code](https://docs.pandorian.ai/essentials/pandorian-integrations#policy-as-code).
...
For Jira setup details, see [Jira Integration](https://docs.pandorian.ai/essentials/pandorian-integrations#jira-integration).
