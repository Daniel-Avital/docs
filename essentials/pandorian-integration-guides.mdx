...
If your team already uses tools like Claude Skills or internal `skills.md` files to guide AI coding assistants, you can sync those same files to Pandorian. This ensures that the rules your AI uses to write code are the exact same rules Pandorian uses to validate code at the Pull Request level.

## How it Works

1. **Source Selection**: You designate a specific repository and a "base directory" (e.g., `/docs/guidelines`) where your standards live.
2. **Automated Sync**: Pandorian monitors this directory for `.md` files.
3. **Enforcement**: Any change to these files automatically updates the active guidelines in Pandorian, which are then enforced across all your associated repositories during scans.
...
1. Navigate to **Settings > Guideline Sync** in the Pandorian Dashboard
