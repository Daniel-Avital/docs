...
Your org runs security and architecture guidelines across all repos. Test files shouldn't be subject to the same enforcement as production code. Add glob patterns like `**.test.ts`, `**.test.js`, `**.cy.js`, and `**/*_test.go` as ignored paths — Pandorian will skip those files across all guidelines without any per-guideline configuration.
