...
Tags serve two purposes: general-purpose filtering/organization, and **guideline grouping** for holistic analysis.

#### General Tags

Tags are free-form strings you can use to filter and organize guidelines. They're used in Policy as Code to control which guidelines apply to specific repositories in addition to filtering and organizing.
...
Tags that start with `grouping:` have special behavior. They tell Pandorian to analyze the tagged guidelines **together as a unified guideline set** rather than independently.

**Format:** `grouping:<group_name>`

```yaml
# guideline-a.md
pandorian-metadata:
  tags: [grouping:error-handling]
...
During analysis, guidelines that share the same `grouping:` tag are evaluated in the model together. The model evaluates the code against all guidelines in the group holistically. Violations are still reported under each individual guideline.

**When to use grouping:**

Use grouping when you have guidelines that are semantically related and understanding one is important for correctly evaluating another. For example, a set of guidelines about error handling patterns where "wrap errors with context" and "use structured logging for errors" should be considered together — a function that wraps errors correctly but logs them incorrectly should be evaluated with full awareness of both guidelines.

**A guideline can belong to multiple groups:**

```yaml
pandorian-metadata:
  tags: [grouping:error-handling, grouping:observability]
```

This guideline will appear in both the `error-handling` group analysis and the `observability` group analysis.
...
pandorian-metadata:
  ...
  tags: [grouping:api-security, v2]
---
All API endpoints must validate and sanitize input parameters...
```

Both guidelines share `grouping:api-security`, so when Pandorian analyzes a code snippet, it evaluates both rules together. The model sees both guidelines in the same analysis and can reason about their combined intent.
Apollo
