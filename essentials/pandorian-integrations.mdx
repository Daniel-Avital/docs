...
The exported file carries over `language_ids`, `title`, `block`, `category`,
`is_active`, and `tags` exactly as configured in the UI — no manual
reformatting required.

Exported files are named `{id}_{timestamp}.md` (e.g.
`SEC_84721_20260827123823.md`). Before committing, rename the file to match
the guideline `id` (e.g. `SEC_84721.md`) so that re-exporting the same
guideline later overwrites it rather than adding a duplicate.
