...
Scan Policies let you define exactly where and how guidelines run — which paths to scan, which paths to ignore, and which guideline groups to scope to specific files or directories. Repo-level overrides take precedence over org-level defaults.
...
**Included Paths:** The paths Pandorian scans, applied across all guidelines. Use this when enforcement should only run against a subset of the codebase — for example, application code only, excluding infrastructure-as-code, docs, or generated directories. Setting Included Paths also reduces scan time, since Pandorian only analyzes those paths instead of the entire repository. Click **Include a path** to add one. If no included paths are set, Pandorian scans the entire repository by default, subject to Ignored Paths.
...
> **Note:** Pandorian applies path rules in a fixed order: **Included Paths** first, then **Guideline Scoping**, then **Ignored Paths**.
>
> Included Paths sets the overall surface Pandorian scans at all — this is evaluated first, and is also what keeps scans fast, since nothing outside it is analyzed. Guideline Scoping is then evaluated within that surface, narrowing which paths a specific guideline group applies to — it can only narrow enforcement within what's already included, it can't extend enforcement to a path that scanning doesn't cover. Ignored Paths is applied last, excluding specific files or directories from whatever remains, even if they sit inside an Included Path or a scoping rule.
>
> Because Guideline Scoping is evaluated within the Included Paths surface rather than independently of it, the two need to stay aligned. If Included Paths is set to path A, and a scoping rule targets path B outside of A, that guideline group will never run — path B is never scanned in the first place, so the scoping rule has nothing to act on. When adding a scoping rule, check that its path falls within your Included Paths, or that no Included Paths restriction is set.

#### Repository Overrides

Repository overrides let you customize scan policy for a specific repository, overriding the organization defaults — including its own Included Paths, Ignored Paths, and scoping rules. This is useful when a particular repo has different conventions, third-party code that should be excluded, or guidelines that only apply in that context. Click **Add repository override** to configure.

#### Examples

**Scanning only application code:**
Your repository contains application code alongside infrastructure-as-code and documentation. Set Included Paths to `/src` so only application code is scanned, keeping Terraform files and docs out of scope entirely.
...
**Avoiding a misaligned scope:**
Included Paths is set to `/src`, but a scoping rule assigns the `auth` tag to `/services/auth`, which lives outside `/src`. Because `/services/auth` is never scanned, the `auth` guidelines never actually run there — despite the scoping rule looking correctly configured. Either widen Included Paths to cover `/services/auth`, or point the scoping rule at a path within `/src`.
