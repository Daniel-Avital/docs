...
The Library gives you a starting point for common engineering standards across supported languages and categories. It is organized into Guidelines, individual standards you can add on their own, and Packages, curated guideline sets built around common use cases across domains such as Cloud Platforms, Containers/Kubernetes/IaC, Data and Storage, Messaging/Events/Jobs, APIs, Security, Compliance Frameworks, Regulatory Regimes, Industry, Architecture/Quality/Reliability/Observability, and language-specific best practices.

To install a package:

1. Navigate to the Library page.
2. Browse or search for a package.
3. Click the package to view its contents.
4. Install the entire package, or select individual guidelines from within it.
5. Select the language you want the guidelines to run on.
6. After adding the guidelines, you can navigate to the guidelines page and choose the enforcement mode for each guideline you added.
7. Add tags if you want to use them with scan policies.

You can browse and add individual guidelines the same way, without installing a full package.

When you install a guideline from the Library, it becomes a guideline in your organization. It does not remain linked back to the Library, and it is fully editable so you can tailor it to your codebase. You can configure how it should behave in your environment, including whether it should run in Monitor mode or Block Build mode.

Library guidelines cannot be scoped directly from within the Library itself. To scope them, add useful tags and then use Scan Policies to decide where those tagged guidelines should run.
...
**Pick the guidelines.** Guidelines attached to an Initiative can come from the library, your own imported standards, or anything written from scratch. The Initiative only needs to know which guidelines count toward this specific program.
...
Pandorian supports four scan types:
...
5. For the path, select the full repository (the repository root) — not a folder underneath it. This is what makes it a full scan rather than a targeted one.
6. Start the scan.
...
1. Navigate to the **Repositories** page.
2. Click the **Play** button next to your repository.
3. Select the guideline and the path you want to scan.
4. Choose whether to apply your scan policies using the **scan policies** toggle (see below).
5. Start the scan.

By default, a targeted scan respects your configured scan policies — Included Paths, Guideline Scoping, and Excluded Paths. Turn the **scan policies** toggle off to ignore those policies and scan the exact path you selected regardless of them, which is useful when you want to validate a guideline against a path that your policies would normally exclude or scope away. Leave it on to have the targeted scan behave consistently with your full repository and PR scans.
...
Scheduled scans run a repository scan automatically on a recurring schedule you define, so you get ongoing visibility into guideline compliance without kicking off a scan by hand each time.

You configure scheduled scans from the **Repositories** page. Next to the **Play** button for a repository, click the **calendar** button to open the **Scheduled Scans** page for that repository, where you can review existing scheduled scans and add new ones.

#### Adding a scheduled scan

On the Scheduled Scans page, add a scheduled scan and configure it:

1. **Name** — Give the scheduled scan a name so you can recognize it later.
2. **Schedule** — Define when it runs using a cron expression. As you type, live text below the field translates the expression into plain-language run times, so you can confirm the schedule is what you intended before saving.
3. **Guideline and path** — Choose the guideline to scan against and the path to scan, the same way you would for a targeted scan. Select the repository root to scan the full repository, or a folder underneath it to scope the scheduled scan to that path.
4. **Scan policies** — Decide whether the scheduled scan should apply your configured scan policies (Included Paths, Guideline Scoping, and Excluded Paths) or ignore them, using the same toggle as a targeted scan.

Once saved, the scan runs automatically on the schedule you defined. Results appear on the **Scans** page alongside your manual and PR scans.
...
its own guidelines, scan history, and UI, all managed from that one account. 
