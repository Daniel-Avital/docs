...
Scan Policies let you define exactly which files Pandorian scans, which guidelines apply to them, and which files are excluded. Rules exist at both the repository and organization level, and are evaluated in a fixed order — the first rule that applies to a file decides its outcome.
...
2. **Repository-level Guideline Scoping** — If the file matches a scoping rule configured on this repository, it's scanned, but only with the guidelines named in that rule.
3. **Organization-level Guideline Scoping** — If the file matches a scoping rule configured at the organization level, it's scanned, but only with the guidelines named in that rule.
4. **Repository Ignored Paths** — If the file matches an ignored path configured on this repository, it is not scanned.
5. **Organization Ignored Paths** — If the file matches an ignored path configured at the organization level, it is not scanned.
...
>
> Because Guideline Scoping (steps 2–3) is checked before Ignored Paths (steps 4–5), a file that matches a scoping rule is scanned even if it would otherwise fall under an Ignored Paths rule — the Ignored Paths check is never reached for that file. If a path needs to be excluded no matter what, use Repository Included Paths (step 1); it's the only check nothing later can override.

#### Path evaluation order at a glance

```mermaid
flowchart TD
    A[File f in repo r] --> B{Repo has Included Paths<br/>and f is outside them?}
    B -->|Yes| B1[Not scanned]
    B -->|No| C{f matches a repo-level<br/>Guideline Scoping rule?}
    C -->|Yes| C1[Scanned with that rule's guidelines]
    C -->|No| D{f matches an org-level<br/>Guideline Scoping rule?}
    D -->|Yes| D1[Scanned with that rule's guidelines]
    D -->|No| E{f matches a repo-level<br/>Ignored Paths rule?}
    E -->|Yes| E1[Not scanned]
    E -->|No| F{f matches an org-level<br/>Ignored Paths rule?}
...
Organization-level policy applies across all repositories by default, and is checked after any repository-level rules.

**Guideline Scoping:** Restrict a guideline group to specific paths across the organization. Rather than running all guidelines across the entire codebase, scoping rules let you target enforcement by tag — for example, applying security guidelines only to the auth service, or data guidelines only to migration files. Click **Add scoping rule** to configure.

**Ignored Paths:** Paths that should never be scanned, applied org-wide — for files that didn't already match a Guideline Scoping rule first. Use this to exclude vendor code, generated files, test fixtures, or any directory that shouldn't be subject to enforcement. Click **Ignore a path** to add an exclusion.

#### Repository Overrides

Repository overrides customize scan policy for a specific repository. Every repository-level rule is checked before organization-level rules, so it takes precedence. Click **Add repository override** to configure.
...
**Guideline Scoping:** The same scoping mechanism as the organization level, but scoped to this repository only, and checked before organization-level scoping rules.

**Ignored Paths:** The same exclusion mechanism as the organization level, but scoped to this repository only, and checked before organization-level ignored paths — and, like all Ignored Paths rules, only reached if the file didn't already match a Guideline Scoping rule.

#### Examples

**Limiting a noisy repo's scan surface:**
A repository contains generated code alongside application code. Set that repository's Included Paths to `/src` so nothing outside it is ever scanned — cutting scan time and noise regardless of any other rule.

**Scoping compliance guidelines to sensitive services, org-wide:**
Your organization has a set of guidelines tagged `pci-compliance`. Create an organization-level scoping rule restricting them to repos or paths containing payment processing code. Every other repo is unaffected, and matching files are scanned with just those guidelines.

**Scoping security guidelines to one repo's auth layer:**
Security guidelines tagged `auth` should only apply to `/src/auth` in a single repository, not organization-wide. Add a repository-level scoping rule instead of an organization-level one — it's checked first and won't affect any other repo.

**A scoping rule reaching a file you meant to ignore:**
Your organization ignores `**/generated/**` org-wide. A repository-level scoping rule tags everything under `/src` with `code-quality` guidelines, and `/src/generated` happens to fall inside that path. Because Guideline Scoping is checked before Ignored Paths, files in `/src/generated` are scanned with the `code-quality` guidelines — the org-wide ignore rule is never reached for them. To exclude `/src/generated` no matter what, keep it outside that repository's Included Paths instead.

**Overriding org defaults for a legacy repository:**
Your org-wide policy ignores test files. A legacy repo requires the opposite — tests are the only place certain patterns are enforced. Add a repository-level Ignored Paths override for that repo's non-test files, without affecting the rest of the organization.
