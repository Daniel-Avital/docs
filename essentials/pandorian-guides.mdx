...
4. **Repository Excluded Paths** — If the file matches an excluded path configured on this repository, it is not scanned.
5. **Organization Excluded Paths** — If the file matches an excluded path configured at the organization level, it is not scanned.
...
flowchart TD
    ...
    D -->|No| E{f matches a repo-level<br/>Excluded Paths rule?}
    E -->|Yes| E1[Not scanned]
    E -->|No| F{f matches an org-level<br/>Excluded Paths rule?}
...
**Excluded Paths:** Paths that should never be scanned, applied org-wide — for files that didn't already match a Guideline Scoping rule first. Use this to exclude vendor code, generated files, test fixtures, or any directory that shouldn't be subject to enforcement. Click **Exclude a path** to add an exclusion.
...
**Excluded Paths:** The same exclusion mechanism as the organization level, but scoped to this repository only, and checked before organization-level excluded paths — and, like all Excluded Paths rules, only reached if the file didn't already match a Guideline Scoping rule.
...
**A scoping rule reaching a file you meant to exclude:**
Your organization excludes `**/generated/**` org-wide. A repository-level scoping rule tags everything under `/src` with `code-quality` guidelines, and `/src/generated` happens to fall inside that path. Because Guideline Scoping is checked before Excluded Paths, files in `/src/generated` are scanned with the `code-quality` guidelines — the org-wide exclusion rule is never reached for them. To exclude `/src/generated` no matter what, keep it outside that repository's Included Paths instead.
